...
- **Backend Anti-Abuse (Strict 24h IP-Limits):** Um massenhaftes Spammen von Gutscheinen zu verhindern, speichert die Tabelle `winners` nun die `user_ip`. Das Backup-Limit (`spinLimiter`) blockt auf primitiver HTTP Ebene maximal 3 Versuche pro 24H. Der **Gnadenlose DB-Check** in `/api/spin` prüft aber hart, ob die abgefragte IP in den letzten 24 Stunden bereits einen Preis gewonnen hat (unabhängig von Browser/Cash-Resets). Die Fehlermeldungen sind spielerfreundlich formatiert ("Du hast bereits am Gewinnspiel teilgenommen...").
- **Admin: Gewinner Manuell Löschen:** Im Admin Dashboard können "versehentliche" Gewinner in der Tabelle gelöscht werden (Mülleimer-Symbol). Dies ist **ausschließlich** möglich, solange `email_sent === 0` ist. Beim Löschen gibt das System den ERP-Gutscheincode dynamisch wieder in den Pool zurück (`UPDATE prize_codes SET is_used = 0`), damit er dem nächsten echten Kunden gezogen werden kann.
- **Admin Test-Modus:** Um selbst ungestört testen zu können, befindet sich unter `Settings` im Admin-Dashboard ein "Test-Modus" Toggle. Sobald dieser via Checkbox aktiviert wird, bypassen alle API Routen das IP-Limit und den E-Mail Check. Dieser Toggle speichert sich selbstständig beim Klick (Auto-Save on Change).
- **Vite & Express static fallback:** Im Docker-Production Mode (gesteuert durch `ENV NODE_ENV=production` im Dockerfile) läuft NICHT mehr der langsame Vite-Hot-Module-Reload Server (der WebSocket Errors wirft), sondern `express.static(path.join(__dirname, 'dist'))` serviert die gebauten Files rasend schnell auf Port 3000.
- **Mac Shortcut Reset:** Der versteckte Keyboard-Shortcut um den Browser-LocalStorage "Gespielt" Marker komplett zurückzusetzen lautet `Option/Alt + Shift + R`. Dies trigger `e.code === 'KeyR'` anstelle von `e.key`, da MacBooks beim Drücken von Shift+Option spezielle Sonderzeichen ins `e.key` Event feuern.
